 ...
 *  3. corner case handling: remaining work that is less than one block is allocated to the last thread
 */
inline void partition_workload_by_thread(
    int64_t total_elems, 
    int64_t block_size, 
    int64_t &range_start, 
    int64_t &range_end) {
    ...
    if (thread_id < num_lo_threads_extra_block) {
        ...
        range_start = hi_group_start + (thread_id - num_lo_threads_extra_block) * workload_per_hi_thread;
        range_end = hi_group_start + (thread_id - num_lo_threads_extra_block + 1) * workload_per_hi_thread;

        // if it is the last thread, allocate the remaining amount that is less than one block to it as well
